    ...
    <Card className="gap-0">
      <CardHeader className="border-b">
        ...
        <CardDescription className="flex flex-wrap items-center gap-x-3 gap-y-1">
          <span>{feature.project ?? 'No project'}</span>
          <span>Updated {formatDateTime(feature.updated_at)}</span>
        </CardDescription>
        <CardAction className="flex items-center gap-2">
          ...
          {feature.proposal_id ? (
            <Badge variant="outline">Proposal {shortId(feature.proposal_id)}</Badge>
          ) : null}
        </CardAction>
      </CardHeader>
          ...
          {items.length === 0 ? (
            ...
            items.map(item => {
              ...
              return (
                <div
                  key={item.id}
                  className="flex flex-col gap-2 rounded-lg border bg-background/70 p-4"
                >
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
                    {item.task_id ? <span>Task {shortId(item.task_id)}</span> : null}
