...
class AgentTaskRunner:
    ...
    async def retry_failed_task(self, task_id: uuid.UUID) -> uuid.UUID:
        """Clone a failed coding task into a fresh queued task and dispatch it."""
        async with self._database.session() as session:
            source = await TaskRepository.get(session, task_id)
            if source is None:
                raise ValueError("Task not found")
            if source.category != TaskCategory.coding:
                raise ValueError("Only coding tasks can be retried")
