...
PROMPT_COVERAGE_CASES = {
    "workflow": [
        "Standard workflow",
        "Understand the project",
        "Discover gaps",
        "wait for approval",
        "Plan approved proposals",
    ],
    "proposal_template": [
        "title",
        "plan type",
...
def test_marc_system_prompt_covers_workflow_template_and_secret_rules():
    from src.agents.marc.system_prompt import MARC_SYSTEM_PROMPT

    prompt = MARC_SYSTEM_PROMPT.lower()
    missing = {
        section: [text for text in expected_text if text.lower() not in prompt]
        for section, expected_text in PROMPT_COVERAGE_CASES.items()
    }

    assert missing == {section: [] for section in PROMPT_COVERAGE_CASES}
